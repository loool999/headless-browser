...
def start_browser():
    try:
        success = asyncio.run(init_browser())
...
def get_content():
    try:
        ...
        text = asyncio.run(page.evaluate("() => document.body.innerText"))
        title = asyncio.run(page.title())
...
def execute_js():
    try:
        ...
        result = asyncio.run(page.evaluate(script))
